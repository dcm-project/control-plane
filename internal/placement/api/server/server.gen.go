...
func (siw *ServerInterfaceWrapper) RehydrateResource(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.RehydrateResource(w, r, resourceId)
	}))
...
func HandlerWithOptions(si ServerInterface, options ChiServerOptions) http.Handler {
	...
	r.Group(func(r chi.Router) {
		r.Post(options.BaseURL+"/resources/{resourceId}:rehydrate", wrapper.RehydrateResource)
	})
...
// RehydrateResource operation middleware
func (sh *strictHandler) RehydrateResource(w http.ResponseWriter, r *http.Request, resourceId ResourceIdPath) {
	var request RehydrateResourceRequestObject

	request.ResourceId = resourceId

	var body RehydrateResourceJSONRequestBody
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		sh.options.RequestErrorHandlerFunc(w, r, fmt.Errorf("can't decode JSON body: %w", err))
		return
	}
	...
	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.RehydrateResource(ctx, request.(RehydrateResourceRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "RehydrateResource")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(RehydrateResourceResponseObject); ok {
		if err := validResponse.VisitRehydrateResourceResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
